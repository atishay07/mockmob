...
export function normalizeGenerationPayload(payload, context = {}) {
  ...
  for (const group of groups) {
    console.log('[passage_group] normalized', {
      ...
      linked_question_count: questions.filter((question) => question.temporary_group_key === group.temporary_group_key).length,
...
export function flattenPassageGroup(group, context = {}, groupIndex = 1) {
  ...
  if (!group || typeof group !== 'object') return { questions: [], groups: [], rejected: 1 };
  ...
  const temporaryGroupKey = String(group.passage_group_id || group.group_id || `tmp_${passageId}_${groupIndex}`).trim();
  const children = Array.isArray(group.questions) ? group.questions : [];
  if (!passageText || !passageId || children.length === 0) {
    return { questions: [], groups: [], rejected: Math.max(1, children.length) };
  }
...
export function canPublishPassageGroup(validLinkedQuestions, group = {}) {
